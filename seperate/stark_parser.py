...
def stark_parser(can_ids, vehicle_data):
    ...
    if vehicle_data:
        for can_id, can_data in zip(can_ids, vehicle_data):
            ...
            if can_id == "110":
            ...
            elif can_id == "113" or can_id == "114" or can_id == "115":
                try:
            ...
            elif can_id == "116" or can_id == "117" or can_id == "118":
                try:
                ...
                '''elif can_id == "11C":
                    try:
                        bmsData["BatteryCapacity_Ah"] = convert_and_get_desired_value(
                        ...
                        bmsData["BUILD_Date"] = convert_and_get_desired_value(
                            can_data[12:14]
                        )
                        bmsData["hw_pcb"] = convert_and_get_desired_value(can_data[14:])
                    except:
                        bmsData["BatteryCapacity_Ah"] = bmsData["FW_major"] = bmsData[
                            "FW_minor"
                        ] = bmsData["BUILD_Date"] = bmsData["hw_pcb"] = 0'''            
            elif can_id == "11C":
                try:
                ...
                try:
                    ...
                    ControllerDATA[
                        "ThrottleMultiplier"
                    ] = convert_and_get_desired_value(string=can_data[2:4])
        ...
        bmsData["Cell_V_Max_Val"] = max(
            bmsData["CMU1_Cell_Vtgs"] + bmsData["CMU2_Cell_Vtgs"]
        )
        bmsData["Cell_V_Min_Val"] = (
            min(
                list(filter(lambda x: x > 100, bmsData["CMU1_Cell_Vtgs"]))
                + list(filter(lambda x: x > 100, bmsData["CMU2_Cell_Vtgs"]))
            )
            if bmsData["Cell_V_Max_Val"]
            else 0
        )
    req_tuple = []
    ...
    # print(str("         "),ControllerDATA["ControllerMainState"], ControllerDATA["MotorTorqueEstimated"], ControllerDATA["BatteryPowerConsumed"])
    # print(expected_output)
    # print(bmsData["CMU1_Cell_Vtgs"][0])
