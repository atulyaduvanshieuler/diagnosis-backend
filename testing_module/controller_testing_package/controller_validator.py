...
def controller_validation_function(controller_output):
    ...
    try:
        ...
        if float(controller_output_list[1]) < -50 or float(controller_output_list[1]) > 200:
            ...
            controller_data_logger.error(error_string)
        ...
        controller_general_logger.info("DIAG_CONTROLLER_STOP command ran in controller_validation_function exception handling with following exception: %s" %e)
        return False
